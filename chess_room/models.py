...
class Tournament(models.Model):
    ...
    tournament = models.ForeignKey(
        "self", related_name="parent", on_delete=models.CASCADE, blank=True, null=True
    )
    ...
    players = models.ManyToManyField(
        User, verbose_name=_("Участники"), blank=True, null=True
    )
    closing_date = models.DateField(
        verbose_name=_("Дата закрытия регистрации"), default=timezone.now()
    )
    closing_time = models.TimeField(
        verbose_name=_("Время закрытия регистрации"), default=timezone.now()
    )
...
class TournamentResult(models.Model):
    ...
    player = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="player_user"
    )
