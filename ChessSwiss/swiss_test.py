...
if len(players) % 2 == 0:
    for i in range(len(players) // 2):
        ...
        if rez == 1:
            ...
            p1 = 0.5
            p2 = 0.5
        turnir[players[i]][1].extend(
            [players[len(players) // 2 + i], p1, "w", "", 0, p1]
        )
        turnir[players[len(players) // 2 + i]][1].extend(
            [players[i], p2, "b", "", 0, p2]
        )
    number_tur += 1
    ...
    turnir[players[len(players) - 1]][1].extend(
        [players[len(players) - 1], 1, "", "", 1]
    )
    number_tur += 1
    ...
    while len(pars) != len(players) // 2:
        ...
        if j < len(players):
            ...
            if (
                i != j
                and flag
                and turnir[i][0] not in add_player
                and turnir[j][0] not in add_player
            ):
